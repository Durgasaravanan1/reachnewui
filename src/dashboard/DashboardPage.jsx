...
const RECENT_ACTIVITY = [
  {
    ...
    content: (
      <>
        <span className="font-bold text-slate-800">April Newsletter</span> sent to 8,230 contacts
      </>
    ),
    time: "2h ago",
  },
  {
    ...
    content: (
      <>
        <span className="font-bold text-slate-800">WhatsApp Flash Sale</span> completed — 68.3% read rate
      </>
    ),
    time: "5h ago",
  },
  {
    ...
    content: (
      <>
        Imported <span className="font-bold text-slate-800">342 contacts</span> into "Active Customers"
      </>
    ),
    time: "1d ago",
  },
  {
    ...
    content: (
      <>
        <span className="font-bold text-slate-800">Re-engagement Series</span> scheduled for May 1
      </>
    ),
    time: "1d ago",
  },
  {
    ...
    content: (
      <>
        New template <span className="font-bold text-slate-800">"Product Update May"</span> created
      </>
    ),
    time: "2d ago",
  },
...
export default function DashboardPage() {
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-screen">
      ...
      {/* Alert - FIXED: now navigates to the correct campaign detail page */}
      {alertVisible && (
        <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 p-4 flex items-start justify-between gap-3">
          <div className="flex items-start gap-3">
            <span className="text-base mt-0.5">⚠️</span>
            <div>
              ...
              <p className="text-sm text-amber-800 mt-0.5">
                "April Newsletter" has a hard bounce rate of 5.8%, above your 5% threshold.{" "}
                <button
                  onClick={() => navigate("/campaigns/1")}   // ✅ fixed: goes to April Newsletter detail
                  className="font-bold underline text-amber-900 hover:text-amber-800"
                >
                  Review campaign →
                </button>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {ACTIVE_CAMPAIGNS.map((c) => (
                    <tr
                      key={c.id}
                      onClick={() => navigate(`/campaigns/${c.id}`)}   // ✅ navigates to detail page
                      className="border-b border-slate-50 last:border-0 hover:bg-slate-50 transition-colors cursor-pointer"
                    >
                      <td className="py-3 pr-3">
                        <div className="flex items-center gap-2.5">
