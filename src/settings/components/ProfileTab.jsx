// ProfileTab.jsx - Professional Enterprise Grade
...
const Button = ({ children, variant = 'primary', size = 'md', leftIcon, rightIcon, onClick, disabled, loading, className }) => {
  const base = "inline-flex items-center gap-2 rounded-lg font-medium transition-all focus:outline-none focus:ring-2 focus:ring-offset-1 disabled:opacity-50 disabled:cursor-not-allowed";
  const variants = {
...
const Input = ({ label, value, onChange, type = 'text', placeholder, disabled, error, required, helpText }) => (
  <div>
    {label && (
    ...
    <input
      ...
      disabled={disabled}
      className={cn(
        "w-full rounded-lg border bg-white px-3.5 py-2.5 text-sm transition-all focus:outline-none focus:ring-2",
        error 
...
const Select = ({ label, value, onChange, options, disabled }) => (
  <div>
    ...
    <select
      ...
      onChange={onChange}
      disabled={disabled}
      className="w-full rounded-lg border border-slate-200 bg-white px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 transition-all disabled:bg-slate-50"
    >
      {options.map(opt => (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm">
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
              <Button variant="secondary" onClick={() => setView('profile')}>Cancel</Button>
              <Button variant="primary" onClick={handleUpdatePassword} loading={isSaving}>
                Update Password
              </Button>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  className="w-full rounded-lg border border-slate-200 bg-white px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 transition-all resize-none"
                />
              </div>
            </div>
            <div className="flex justify-end gap-3 pt-6 mt-4 border-t border-slate-100">
              <Button variant="secondary" onClick={() => setView('profile')}>Discard</Button>
        ...
        <div className="px-6 pb-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between -mt-12 mb-6">
            <div className="relative">
              <div 
                className="w-28 h-28 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-3xl font-bold shadow-lg cursor-pointer hover:opacity-90 transition-opacity ring-4 ring-white"
              ...
              <button 
                onClick={handleAvatarClick}
                className="absolute bottom-1 right-1 p-1.5 bg-white rounded-full shadow-md border border-slate-200 hover:bg-slate-50 transition-colors"
              >
                <CameraIcon />
              </button>
            ...
            <div className="flex gap-2 mt-4 sm:mt-0">
              <Button variant="secondary" leftIcon={<EditIcon />} onClick={() => setView('edit')}>
                Edit Profile
              </Button>
              <Button variant="ghost" leftIcon={<LockIcon />} onClick={() => setView('password')}>
                Change Password
              </Button>
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            {profile.bio && (
              <p className="text-sm text-slate-600 mt-4 max-w-2xl">{profile.bio}</p>
            )}
          </div>
