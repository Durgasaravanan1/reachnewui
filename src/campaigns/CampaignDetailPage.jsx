...
const generateMockAnalytics = (campaignId) => {
  const analytics = {
    '1': { sent: 8230, delivered: 7980, deliveryRate: 0.97, uniqueOpens: 3180, openRate: 0.387, uniqueClicks: 920, ctr: 0.112, unsubscribes: 28, bounceRate: 0.032 },
    '2': { sent: 3180, delivered: 3160, deliveryRate: 0.994, uniqueOpens: 2170, openRate: 0.683, uniqueClicks: 890, ctr: 0.28, unsubscribes: 12, bounceRate: 0.006 },
    '3': { sent: 0, delivered: 0, deliveryRate: 0, uniqueOpens: 0, openRate: 0, uniqueClicks: 0, ctr: 0, unsubscribes: 0, bounceRate: 0 },
  };
  return analytics[campaignId] || analytics['1'];
};

const generateMockLinks = (campaignId) => {
  if (campaignId === '1') return [
    { linkLabel: 'April Special', linkUrl: 'https://example.com/april', totalClicks: 520, ctr: 6.32 },
    { linkLabel: 'New Arrivals', linkUrl: 'https://example.com/new', totalClicks: 310, ctr: 3.77 },
    { linkLabel: 'Unsubscribe', linkUrl: 'https://example.com/unsub', totalClicks: 28, ctr: 0.34 },
  ];
  if (campaignId === '2') return [
    { linkLabel: 'Shop Now', linkUrl: 'https://example.com/shop', totalClicks: 670, ctr: 21.1 },
    { linkLabel: 'View Deal', linkUrl: 'https://example.com/deal', totalClicks: 210, ctr: 6.6 }
  ];
  return [];
};

const generateMockRecipients = (campaignId) => {
  if (campaignId === '1') return [
    { recipientId: '1', contactName: 'John Doe', lastEvent: 'clicked' },
    { recipientId: '2', contactName: 'Jane Smith', lastEvent: 'opened' },
    { recipientId: '3', contactName: 'Bob Johnson', lastEvent: 'bounced' },
  ...
  if (campaignId === '2') return [
    { recipientId: '101', contactName: 'Maria Garcia', lastEvent: 'clicked' },
    { recipientId: '102', contactName: 'James Brown', lastEvent: 'opened' },
  ];
  return [];
};
...
const useCampaignDetailData = (campaignId) => {
  const [state, setState] = useState({
    campaign: null,
    analytics: null,
    links: [],
  ...
  useEffect(() => {
    const fetchData = async () => {
      await delay(500);
      const campaign = generateMockCampaign(campaignId);
      if (!campaign || campaign.campaignName === 'Unknown Campaign') {
        setState({ ...state, isLoading: false, notFound: true });
        return;
      }
      setState({
        campaign,
        analytics: generateMockAnalytics(campaignId),
        links: generateMockLinks(campaignId),
        recipients: generateMockRecipients(campaignId),
        isLoading: false,
        notFound: false,
      });
    };
...
export default function CampaignDetailPage() {
  ...
  return (
    <div className="p-4 md:p-6">
          ...
          <>
            <div className="flex items-center gap-2">
              ...
              {campaign.scheduledAt && !campaign.sentAt && (
                <span className="text-xs text-slate-400">Scheduled: {formatDateTime(campaign.scheduledAt)}</span>
              )}
            </div>
            <Button variant="secondary" leftIcon={<CopyIcon />} onClick={handleDuplicate}>
              Duplicate
      ...
      {/* Bounce alert (conditional) */}
      {campaign.status === 'sent' && analytics?.bounceRate > 0.03 && (
        <Alert variant="warning" title="Elevated bounce rate detected" className="mb-6">
          Hard bounce rate is above your 5% threshold. Affected contacts have been automatically suppressed.
        </Alert>
      )}
